...
export const staffSchema = z.object({
  name: nameSchema(2, "Staff member name is required"),
  email: emailSchema,
  phone: phoneSchema,
  password: passwordSchema.optional().or(z.literal("")),
  role: optionalTextOnlySchema(100),
  salary: priceSchema("Salary", { allowZero: true }).optional().default(0),
  shift: optionalTextSchema(100),
  dept: optionalTextOnlySchema(100),
  department: optionalTextOnlySchema(100),
...
export const guestProfileSchema = z
  .object({
    name: nameSchema(2, "Guest name is required"),
    email: emailSchema,
    phone: phoneSchema,
    city: optionalCitySchema,
    state: optionalCitySchema,
    country: optionalCitySchema,
  ...
  .superRefine((data, ctx) => {
    const isAadhaar =
      data.idDocType === "Aadhaar Card" ||
      data.idDocType === "aadhaar" ||
      data.idProof === "Aadhaar Card" ||
      Boolean(data.aadhaar);
...
export const adminUserSchema = z.object({
  name: nameSchema(2, "User full name is required"),
  email: emailSchema,
