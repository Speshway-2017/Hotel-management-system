...
// SVGL Brand Vectors
const FacebookIcon = ({ className = "size-4.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
  </svg>
);

const TwitterIcon = ({ className = "size-4.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
  </svg>
);

const InstagramIcon = ({ className = "size-4.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
...
const LinkedinIcon = ({ className = "size-4.5" }) => (
  <svg className={className} viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 grid gap-10 sm:grid-cols-2 md:grid-cols-4 text-left">
        ...
        <div className="space-y-6">
          ...
          <div className="flex gap-4 pt-2">
            {[
              { icon: FacebookIcon, label: "Facebook", href: property?.settings?.facebook || "https://facebook.com" },
              { icon: LinkedinIcon, label: "LinkedIn", href: property?.settings?.linkedin || "https://linkedin.com" },
              { icon: InstagramIcon, label: "Instagram", href: property?.settings?.instagram || "https://instagram.com" },
              { icon: TwitterIcon, label: "Twitter", href: property?.settings?.twitter || "https://twitter.com" }
            ].map((soc, i) => {
              const Icon = soc.icon;
              return (
                <a 
