...
function EditRoomPage() {
    ...
    async function init() {
      try {
        ...
        if (roomsRes.success && roomsRes.data) {
          const matched = roomsRes.data.find(r => r._id === id || r.roomNumber === id);
          if (matched) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Update room in MERN backend database collection
      await adminService.updateRoom(id, {
        roomNumber,
        category: finalCategory,
        status
      });
      ...
      toast.error(err.message || "Failed to update room configuration");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="space-y-6 text-left font-sans animate-fade-in font-ui">
      <div className="space-y-3.5">
        ...
        <PageHeader
          ...
          subtitle={`Modify parameter definitions or floor maps for room key #${roomNumber}.`}
        />
      </div>
