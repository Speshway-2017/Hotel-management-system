...
function AdminExtendReservation() {
  ...
  const loadBooking = async (isSilent = false) => {
    ...
    try {
      ...
      if (!matched && targetId) {
        const adminRes = await adminService.getReservationById(targetId).catch(() => null);
        if (adminRes && adminRes.data) matched = adminRes.data;
      }
...
export const Route = {
  head: () => ({
    meta: [
      { title: "Extend Stay — Admin Console" },
