...
function AdminExtendReservation() {
  ...
  const handleConfirmExtend = async (e) => {
    ...
    try {
      ...
      let res;
      try {
        res = await adminService.extendReservation(bookingTargetId, payload);
      } catch {
      ...
      if (res && res.success) {
        ...
        navigate({ to: `/admin/reservations/view/${bookingTargetId}` });
      } else {
  ...
  if (error || !booking) {
    return (
      <div className="p-6 max-w-5xl mx-auto text-left font-ui">
        ...
        <div className="mt-4">
          <Button onClick={() => navigate({ to: "/admin/reservations" })} variant="outline" className="text-xs">
    ...
    <div className="p-6 max-w-6xl mx-auto text-left font-ui space-y-6">
      ...
      <Crumbs
        items={[
          ...
          { label: booking.bookingId || "Booking", to: `/admin/reservations/view/${booking._id || booking.id || booking.bookingId}` },
          { label: "Extend Stay" }
        ]}
      />
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-6 rounded-2xl border border-muted shadow-soft">
        ...
        <Button
          type="button"
          onClick={() => navigate({ to: `/admin/reservations/view/${booking._id || booking.id || booking.bookingId}` })}
          variant="outline"
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-muted shadow-soft space-y-5">
            ...
            <div className="pt-2">
              ...
              <Textarea
                id="notes"
                placeholder="e.g., Corporate guest requested stay prolongation for business meetings."
                value={notes}
...
export const Route = {
  head: () => ({
    meta: [
      { title: "Extend Stay — Admin Dashboard" },
  ...
  component: AdminExtendReservation
};

export default AdminExtendReservation;
